...
import { Users, UserCheck, Clock, CalendarOff, TrendingUp, Sprout, Sunrise, PartyPopper, X, Briefcase } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    async function fetchDashboardData() {
        try {
            ...
            // Count unique employees present today (not raw records, since multi-shift creates multiple docs)
            const presentUserIds = new Set();
            attendanceSnap.forEach(doc => presentUserIds.add(doc.data().userId));
            ...
            // Count unique employees currently on bidding (active bidding session today)
            const biddingUserIds = new Set();
            attendanceSnap.forEach(doc => {
                const d = doc.data();
                if (d.type === 'bidding' && d.timeOut === null) biddingUserIds.add(d.userId);
            });
            const onBiddingCount = biddingUserIds.size;

            setStats({ totalEmployees, presentToday, onLeave, pendingRequests, onBidding: onBiddingCount });
            ...
            // Weekly chart: count unique employees per day (not raw records)
            const weeklyUsersByDay = [new Set(), new Set(), new Set(), new Set(), new Set()];
            weekAttendance.forEach((doc) => {
                const data = doc.data();
                const d = data.date?.toDate?.() || new Date(data.date);
                const dayIndex = (d.getDay() + 6) % 7; // Mon=0 ... Fri=4
                if (dayIndex < 5) weeklyUsersByDay[dayIndex].add(data.userId);
            });
            const present = weeklyUsersByDay.map(s => s.size);
    ...
    return (
        <div>
            ...
            <div className="stats-grid">
                ...
                <div
                    className="stat-card clickable"
                    onClick={() => navigate('/admin/bidding')}
                    style={{ cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s' }}
                >
                    <div className="stat-card-header">
                        <div className="stat-card-icon purple"><Briefcase size={24} /></div>
                    </div>
                    <div className="stat-card-value">{stats.onBidding}</div>
                    <div className="stat-card-label">On Bidding Today</div>
                </div>
                <div
                    className="stat-card clickable"
                                    ...
                                    recentAttendance.map((r) => (
                                        <tr key={r.id}>
                                            <td style={{ fontWeight: 600 }}>
                                                {r.name}
                                                {r.type === 'bidding' && <span className="badge badge-info" style={{ marginLeft: 8, fontSize: '0.7rem' }}>Bidding</span>}
                                            </td>
                                            <td>{r.date}</td>
