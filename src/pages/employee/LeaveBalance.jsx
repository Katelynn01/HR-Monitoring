...
export default function LeaveBalance() {
    ...
    async function handleSubmit(e) {
        ...
        setSubmitting(true);
        try {
            // Check for overlap with Bidding Events
            const biddingSnap = await getDocs(
                query(collection(db, 'biddingEvents'), where('assignedEmployees', 'array-contains', user.uid))
            );
            
            const startD = new Date(localStartDate);
            startD.setHours(0,0,0,0);
            const endD = new Date(localEndDate);
            endD.setHours(23,59,59,999);
            
            let overlapEvent = null;
            biddingSnap.forEach(d => {
                const bEvent = d.data();
                const bDate = bEvent.date?.toDate?.() || new Date(bEvent.date);
                if (bDate >= startD && bDate <= endD) {
                    overlapEvent = bEvent.name;
                }
            });

            if (overlapEvent) {
                setFormError(`Leave request overlaps with a required Bidding Event: ${overlapEvent}. Please adjust your dates.`);
                setSubmitting(false);
                return;
            }
